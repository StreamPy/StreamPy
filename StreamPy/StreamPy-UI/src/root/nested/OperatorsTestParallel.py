...
def element_agent(f, inputs, outputs, state, call_streams,
    ...
    def transition(in_lists, state):
        if not in_lists:
            # This agent is a source. It has no input streams. 
            if state:
                v = f(state)
            else:
                v = f()
            # The output is a list of length num_outputs
            # and each element of the output is itself a
            # list which is the sequence sent along the
            # corresponding stream.
            output_lists = \
              [remove_novalue_and_open_multivalue([l]) for l in v]
...
def asynch_element_agent(
    ...
    num_outputs = len(outputs)
    assert_is_list_of_streams_or_None(call_streams)
    assert_is_list_of_streams(inputs)
    assert_is_list_of_streams(outputs)

    def transition(in_lists, state):
        # output_lists is a list of num_outputs number of lists.
        # output_lists[j] is a list of messages that
        # will be sent on output stream j
        # Initially, output_lists is a list of num_outputs number
        # of empty lists.
        output_lists = []
        for _ in range(num_outputs):
            output_lists.append([])

        # If all the input streams have no new data then return empty lists for
        # each output stream, and leave the state and the starting point
        # for each input stream unchanged.
        ...
        for stream_number, v in enumerate(in_lists):
            ...
            if v.stop > v.start:
                ...
                for element in input_list:
                    ...
                    # Ensure that output_lists_increment is a list
                    if not output_lists_increment:
                        output_lists_increment = []
...
def asynch_element_func(f, inputs, num_outputs, state, call_streams,
                        window_size, step_size):
    outputs = [Stream() for i in range(num_outputs)]
    asynch_element_agent(f, inputs, outputs, state, call_streams,
                  window_size, step_size)
    return outputs
...
def many_to_many_agent(f_type, f, f_args, in_streams, out_streams, state,
                 call_streams, window_size, step_size):
    def g(x, state=None):
        if not f_args:
            if state is None: return f(x)
            ...
            if state is None: return f(x, f_args)
            else:
                output, new_state = f(x, state, f_args)
                return (output, new_state)

    return h_agent(f_type, g, in_streams, out_streams,
...
def merge_agent(f_type, f, f_args, in_streams, out_stream,
                state, call_streams, window_size, step_size):
    ...
    return h_agent(f_type, g, in_streams, [out_stream],
                   state, call_streams, window_size, step_size)
...
def split_agent(f_type, f, f_args, in_stream, out_streams,
                state, call_streams, window_size, step_size):
    def g(x, state=None):
        if not f_args:
            if state is None: return f(x[0])
            ...
            else: return f(x[0], state, f_args)

    return h_agent(f_type, g, [in_stream], out_streams,
            state, call_streams, window_size, step_size)
...
def many_outputs_source_agent(f_type, f, f_args, outputs, state, call_streams,
                        window_size, step_size):
    def g(state=None):
        if not f_args:
            if state is None:
    ...
    return h_agent(f_type, g, [], outputs, state,
                   call_streams, window_size, step_size)
...
def single_input_sink_agent(f_type, f, f_args, in_stream, state, call_streams, window_size, step_size):
    def g(x, state=None):
        if not f_args:
            if state is None: return f(x[0])
            else: return f(x[0], state)
    ...
    return h_agent(
        f_type, g, [in_stream], [], state, call_streams,
        window_size, step_size)


def multiple_inputs_sink_agent(
        f_type, f, f_args, in_streams,
        state, call_streams, window_size, step_size):
    ...
    def g(x, state=None):
        if not f_args:
            ...
            if state is None: return f(x, f_args)
            else: return f(x, state, f_args)

    return h_agent(
        f_type, g, in_streams, [], state, call_streams,
        window_size, step_size)
...
def stream_agent(inputs, outputs, f_type, f, f_args,
    ...
    if not inputs:
        ...
        if not call_streams:
            ## raise TypeError('Expected call_streams to be a nonempty list of streams, not:',
            ##             call_streams)
            #print 'WARNING: The function has no input streams and no call streams.'
            pass
    
        if not num_outputs:
            ## raise TypeError('The function has no input or output streams.')
            #print 'WARNING: The function has no input or output streams.'
            pass
    
        elif num_outputs == 1:
            # No inputs. Single output stream.
            return single_output_source_agent(
                f_type, f, f_args, outputs, state, call_streams,
                window_size, step_size)
        else:
            # No inputs. List of multiple output streams.
        ...
        if num_outputs == 0:
            # Single input stream. No outputs.
            return single_input_sink_agent(f_type, f, f_args, inputs, state, call_streams,
                        window_size, step_size)
        elif num_outputs == 1:
            # Single input stream. Single output stream.
            return op_agent(f_type, f, f_args, inputs, outputs,
                            state, call_streams, window_size, step_size)
        else:
            # Single input stream. List of multiple output streams.
            return split_agent(f_type, f, f_args, inputs, outputs,
                         state, call_streams, window_size, step_size)
        ...
        if num_outputs == 0:
            # sink
            return multiple_inputs_sink_agent(f_type, f, f_args, inputs, state, call_streams,
                        window_size, step_size)
        elif num_outputs == 1:
            # Multiple input streams, single output stream
            return merge_agent(f_type, f, f_args, inputs, outputs,
                         state, call_streams, window_size, step_size)
        else:
            # Multiple input and output streams
            return many_to_many_agent(f_type, f, f_args, inputs, outputs,
                                state, call_streams, window_size, step_size)
