...
## Changed 'is' to '==' in h() and h_agent()
def h(f_type, *args):
    if f_type == 'list':
...
def sink_agent(f_type, f, f_args, in_stream, state, call_streams, window_size, step_size):
    def g(x, state=None):
        if not f_args:
            if state is None: return f(x[0])
            else: return f(x[0], state)
...
def stream_agent(inputs, outputs, f_type, f, f_args,
    ...
    if not inputs:
        ...
        if num_outputs == 0:
            # Single input stream. No outputs.
            return sink_agent(f_type, f, f_args, inputs, state, call_streams,
                        window_size, step_size)
        elif num_outputs == 1:
            # Single input stream. Single output stream.
