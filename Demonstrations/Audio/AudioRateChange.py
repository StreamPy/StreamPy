...
def main():
    ...
    logging.info('Wavefile %s has %d audio channels at %d samples/sec frame rate and %d byte samples', \
             wav_file, num_channels, frame_rate, sample_width)
    ...
    # we package the stream into lists of size CHUNK
    
    chunked_stream = package_into_lists(shift_frequency_stream, CHUNK)
    ...
    # Create the stream which will send audio to the loudspeaker/headphones
    audio_output_stream = stream_to_output(py_audio, formatted_stream, frame_rate=frame_rate)    

    # Start processing by creating the audio stream from the wave file. We insist on a mono stream.
